...
import { getColorBySrsLevel } from "../utils";

const DEFAULT_DECK_ID = "default";
const DEFAULT_DECK_LABEL = "Default";

export const WordDeckAssignmentsReach: React.FC<{ activeCard: Word | null }> = ({ activeCard }) => {
    ...
    const toggleCollapsed = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        e.stopPropagation();
        e.preventDefault();
    ...
    const wordDecks = decks.filter((deck) => customDecks.some((customDeck) => customDeck.id === deck.id));
    if (collapsed) {
        return (
            ...
            >
                <div className="flex items-center justify-between gap-3 cursor-pointer">
                    ...
                    {wordDecks.map((deck) => {
                        return (
                            <span
                                ...
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold border transition-all cursor-pointer  bg-sky-500/10 border-sky-400 text-sky-300"
                            >
                                {deck.name}
                            </span>
                        );
                    ...
                    <div>
                        <p className="text-[10px] uppercase tracking-[0.22em] text-slate-500 font-bold"><LucideChevronDown /></p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="w-full rounded-xl bg-slate-950/45 border border-slate-800/40 p-3 space-y-3 text-left event-none">
            <div className="flex flex-row items-top justify-between gap-3 cursor-pointer" onClick={toggleCollapsed}>
                <div>
                    <p className="text-xs text-slate-400">Decks</p>
                    <p className="text-xs text-slate-500 mt-0.5">Assign this word to one or more decks.</p>
                </div>
                <span className="text-slate-500" ><LucideChevronUp /></span>
            </div>
...
export const WordDeckAssignments: React.FC<{ activeCard: Word | null }> = ({ activeCard }) => {
    ...
    if (collapsed) {
        return (
            <div className="flex items-center gap-2 cursor-pointer rounded-xl text-sm bg-slate-950/45 border border-slate-800/40 p-3 space-y-3 text-left"
                onClick={toggleCollapsed}
            >
               <LucideEdit2 className=" w-4 h-4 text-slate-500" /> Decks
            </div>
        );
    }
        ...
        return (
            <>
                <div className="flex items-center justify-between gap-3 cursor-pointer text-xs ">
                    {assignedDeckIds.length === 0 && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full  font-semibold bg-slate-900/80 border border-slate-700/100 text-slate-300">
                            <Lock className="w-3 h-3 text-slate-500" />
                            {DEFAULT_DECK_LABEL}
                        </span>
                    )}
                    {wordDecks.map((deck) => {
                        return (
                            <span
                                key={deck.id}
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full  border  border-sky-500/30 text-sky-500"
                            >
                                {deck.name}
                            </span>
                        );
                    })}
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full uppercase tracking-widest  ${getColorBySrsLevel(activeCard.srsLevel)}`}>L{activeCard.srsLevel}</span>
