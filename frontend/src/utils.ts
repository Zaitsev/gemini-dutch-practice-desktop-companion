...
export function countCardsByCurrentCategory(items: ReviewItem[]): Record<SRSRating, number> {
    ...
    //calculate total words regardles direction
    //   let tolalWordsCounted: Set<string> = new Set();

    for (const item of items) {
        const category = getCategoryByLevel(getSrsBranch(item.word, item.direction).srsLevel);
        //  for word, regardless of how many directions it has. This ensures that a word appearing in both direct and reverse directions is only counted once.  
        //     tolalWordsCounted.add(item.word.id);    
        counts[category]++;
    }
